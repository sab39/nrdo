...
class When extends Expr {
  ...
  public Object evaluate(Context ctx) throws FileLocatedException, IOException {
    Expr res = doTest(ctx);
    return res != null ? res.evaluate(ctx) : null;
  }

  public void write(Context ctx, Writer out) throws FileLocatedException, IOException {
    Expr res = doTest(ctx);
    if (res != null) res.write(ctx, out);
  }

  Expr doTest(Context ctx) throws FileLocatedException, IOException {
    return Bool.truth(test.evaluate(ctx)) ? value : elseVal;
  }

  When(Expr test, Expr value) {
    this.test = test;
    this.value = value;
  }
}
