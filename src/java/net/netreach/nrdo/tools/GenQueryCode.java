///////////////////////////////////////////////////////////////////////////////
// Class: net.netreach.nrdo.tools.GenQueryCode
//
//      Author: Stuart Ballard <sballard@netreach.net>
//  Created On: 2001/08/31
// Description: Construct a source code file from a query definition.
///////////////////////////////////////////////////////////////////////////////
...
/**
 * Construct a source code file from a query definition. The code is written
...
public class GenQueryCode {
  ...
  /**
   * Load in a query and a template, and produce an output file according to the
   * template.
   * 
   * @param args
  ...
  public static void main(String[] args) throws FileLocatedException, IOException {
    ...
    if (args.length < 1) {
      Output.reportError(null, "Usage: GenQueryCode <config> <query> ...");
      System.exit(1);
    }
  ...
  public static void doMain(Config cfg, String[] args) throws FileLocatedException,
    ...
    Expr template = CGLParser.loadTemplate(cfg.querytemplate);

    // Determine the queries to process.
    List tables = dfnbase.getFromArgs(args);
    List queries = dfnbase.getQueriesFromArgs(args);
    cfgvars.put("alltables", new MappedList(tables));
    cfgvars.put("allqueries", new MappedList(queries));
    cfgvars.put("dependtables", new MappedList(dfnbase.getDependTables()));

    for (Iterator i = queries.iterator(); i.hasNext();) {

      // Load the query definition.
      QueryDef qd = (QueryDef) i.next();

      // Put the query values and the configuration's local definitions into ctx
      Context ctx = new Context(basectx);
      if (qd.age > ctx.age) ctx.age = qd.age;
      ...
      ctx.vars = new Scope(qd.toMap(), baseVars);

      // Use the CGL expression created earlier to process the query.
